...
{
    public partial class BuscaProdutoCFOP : TelaBase.BaseCE
    {
        public BuscaProdutoCFOP()
        {
            InitializeComponent();
        }
        ...
        private List<Comum.Clases.ProdutoCFOP> ProdutosCFOPs = null;
        ...
        private void ConfigurarVisualizacao()
        {
            ...
            if (!Aplicacao.Classes.Util.ValidarPermissao(ControleEstoque.Parametros.Parametros.InformacaoUsuario.Permissoes, Comum.Clases.Constantes.COD_PERMISSAO_PRODUTO_CFOP, Comum.Enumeradores.Enumeradores.TipoAcao.INSERIR))
            {
                this.OcultarItemMenu(string.Empty, string.Empty, btnInserir, true);

            }

            if (!Aplicacao.Classes.Util.ValidarPermissao(ControleEstoque.Parametros.Parametros.InformacaoUsuario.Permissoes, Comum.Clases.Constantes.COD_PERMISSAO_PRODUTO_CFOP, Comum.Enumeradores.Enumeradores.TipoAcao.EXCLUIR))
            {
                colDesativar.Visible = false;
            }

            if (!Aplicacao.Classes.Util.ValidarPermissao(ControleEstoque.Parametros.Parametros.InformacaoUsuario.Permissoes, Comum.Clases.Constantes.COD_PERMISSAO_PRODUTO_CFOP, Comum.Enumeradores.Enumeradores.TipoAcao.MODIFICAR))
            {
        ...
        private void RecuperarCFOPs()
        {
            ProdutosCFOPs = LogicaNegocio.ProdutoCFOP.RecuperarCFOP(txtNome.Text,
        ...
        protected override void PreencherGrid(Boolean ExibirMensagem)
        {
            ...
            if (ProdutosCFOPs != null && ProdutosCFOPs.Count > 0)
            {


                foreach (Comum.Clases.ProdutoCFOP c in ProdutosCFOPs)
                {
                    ...
                    dgvComissoes.Rows[dgvComissoes.Rows.Count - 1].Cells[colCodigoCFOP.Index].Value = c.CFOP;
        ...
        {
            GuardarProdutoCFOP frmCFOP = new GuardarProdutoCFOP(Identificador.Value);

            if (frmCFOP.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                RecuperarCFOPs();
        ...
        {
            LogicaNegocio.ProdutoCFOP.DeletarProdutoCFOP(Identificador.Value, ControleEstoque.Parametros.Parametros.InformacaoUsuario.Login);
        ...
        private void btnInserir_Click(object sender, EventArgs e)
        {
            try
            {
                GuardarProdutoCFOP frmCFOP = new GuardarProdutoCFOP( string.Empty);
                    ...
                    if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
                    {
                        ...
                        if (e.ColumnIndex == colEditar.Index || e.ColumnIndex == colDesativar.Index)
                        {
                            ...
                            if (e.ColumnIndex == colEditar.Index)
                            {
                                ...
                                GuardarProdutoCFOP frmCFOP = new GuardarProdutoCFOP(dgvComissoes.Rows[e.RowIndex].Cells[colIdCor.Index].Value as string);
                            ...
                            else if (e.ColumnIndex == colDesativar.Index)
                            {
                                ...
                                LogicaNegocio.ProdutoCFOP.DeletarProdutoCFOP(dgvComissoes.Rows[e.RowIndex].Cells[colIdCor.Index].Value as string, ControleEstoque.Parametros.Parametros.InformacaoUsuario.Login);
                                base.objNotificacao.ExibirMensagem("Comissão desativada com sucesso.", Controles.UcNotificacao.TipoImagem.SUCESSO);
                                RecuperarCFOPs();
                                ExecutarPreencherGrid(false);
                            }
