...
namespace Informatiz.ControleEstoque.Aplicacao.Telas.TelaBase
{
    public partial class BaseCE : TelaBase.Base
    {
        ...
        public Boolean TelaCarregada { get; set; }
        #endregion

        #region"Metodos"

        public string UsuarioTemPermissaoSupervisorGerente()
        {
            string IdentificadorSupervisor = string.Empty;

            if (!ControleEstoque.Parametros.Parametros.InformacaoUsuario.Supervisor && !ControleEstoque.Parametros.Parametros.InformacaoUsuario.Gerente)
            {
               Telas.ValidarTiposEmpregados.ValidarTipoEmpregado frmValidarTipoEmpregado = new Telas.ValidarTiposEmpregados.ValidarTipoEmpregado(true, false);
                if (AbrirFormulario(frmValidarTipoEmpregado) == DialogResult.OK)
                {
                    if (frmValidarTipoEmpregado.UsuarioTemPermissao)
                    {
                        IdentificadorSupervisor = frmValidarTipoEmpregado.IdentificadorUsuario;
                        return IdentificadorSupervisor;
                    }
                    else
                    {
                        Aplicacao.Classes.Util.ExibirMensagemInformacao("O Usuário informado não tem permissão para realizar esta ação!");
                        return string.Empty;
                    }
                }
            ...
            IdentificadorSupervisor = Parametros.Parametros.InformacaoUsuario.Identificador;
            return IdentificadorSupervisor;
        }

        public void UsuarioTemPermissaoSupervisorGerente(ref string IdentificadorSupervisor, ref string NomeSupervisor)
        {
            if (!ControleEstoque.Parametros.Parametros.InformacaoUsuario.Supervisor && !ControleEstoque.Parametros.Parametros.InformacaoUsuario.Gerente)
            {
                Telas.ValidarTiposEmpregados.ValidarTipoEmpregado frmValidarTipoEmpregado = new Telas.ValidarTiposEmpregados.ValidarTipoEmpregado(true, false);
                if (AbrirFormulario(frmValidarTipoEmpregado) == DialogResult.OK)
                {
                    ...
                    {
                        IdentificadorSupervisor = frmValidarTipoEmpregado.IdentificadorUsuario;
                        NomeSupervisor = frmValidarTipoEmpregado.NomeUsuario;
                        return;
                    }
                    else
                    {
            ...
            IdentificadorSupervisor = Parametros.Parametros.InformacaoUsuario.Identificador;
            NomeSupervisor = Parametros.Parametros.InformacaoUsuario.Nome;
        }
        ...
        protected virtual void DesabilitarControles(List<string> NomeControles, bool Habilitado, SDK.Operacoes.operacao operacao)
        {
        ...
        protected virtual void SetarFocusCarregamento()
        {
        ...
        private void Agente_DesabilitarControles(List<string> NomeControles, bool Habilitado, SDK.Operacoes.operacao operacao)
        {
            try
            {
                ...
                if(Habilitado && !TelaCarregada)
                {
                    SetarFocusCarregamento();
                    TelaCarregada = true;
                }
            }
            catch (Exception ex)
        ...
        private void Agente_StatusOperacao(Exception ex, object objSaida, SDK.Operacoes.operacao operacao, Comum.ParametrosTela.Generica Parametros)
        {
            ...
            {
                Aplicacao.Classes.Util.ExibirMensagemErro(ex1.Descricao);
            }
            catch (Exception ex1)
            {
