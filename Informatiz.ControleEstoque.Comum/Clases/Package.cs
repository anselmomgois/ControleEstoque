    ...
    public class Package
    {
        ...
        private static string ParsePackageCode(CQ dom)
        {
            ...
            if (string.IsNullOrEmpty(code))
                throw new  Execao.ExecaoNegocio(Execao.Constantes.CodigoErro.ERRO_NEGOCIO, "Código da encomenda/pacote não foi encontrado.");
        ...
        private static IEnumerable<PackageTracking> ParseTrackingInfo(CQ dom)
        {
            ...
            if (tableRows.Length == 0)
                throw new Execao.ExecaoNegocio(Execao.Constantes.CodigoErro.ERRO_NEGOCIO, dom.Select("p").Text().RemoveLineEndings());
            ...
            {
                throw new Execao.ExecaoNegocio(Execao.Constantes.CodigoErro.ERRO_NEGOCIO, "Não foi possível converter o pacote/encomenda.");
            }

            if (tracking.Count() == 0)
                throw new Execao.ExecaoNegocio(Execao.Constantes.CodigoErro.ERRO_NEGOCIO, "Rastreamento não encontrado.");
